...
$E(X+Y) = \sum_{z \in \mathbb{R}} P(X+Y=z) z = \sum_{z \in \mathbb{R}} \sum_{x,y \vert{} x+y=z} P(X=x \wedge Y=y) (x+y)=\sum_{x,y \in \mathbb{R}} P(X=x \wedge Y=y) (x+y) =$  
...
W poprzednim wykładzie zdefiniowaliśmy, dla dowolnej dyskretnej zmiennej losowej $X$ i zdarzenia $A$ o niezerowym prawdopodobieństwie, nową zmienną $X\vert{}A$.  

Można oczywiście obliczyć wartość oczekiwaną tak zdefiniowanej zmiennej:  

$E(X\vert{}A) = \sum_{x \in \mathbb{R}} xP((X\vert{}A) = x) = \sum_{x \in \mathbb{R}} xP(X=x\vert{}A)$.
...
$EX = \sum_{k=1}^\infty P(A_k) E(X\vert{}A_k)$.  

**Dowód**  

Na mocy twierdzenia o prawdopodobieństwie całkowitym  

$P(X=x) = \sum_{k=1}^\infty P(A_k) P(X=x\vert{}A_k)$  

dla każdego $k \in \mathbb{N}$ i $x \in \mathbb{R}$. Mnożąc tę tożsamość stronami przez $x$ i sumując po wszystkich $x$ dostajemy tezę:  

$EX = \sum_{x \in \mathbb{R}} \sum_{k=1}^\infty xP(A_k) P(X=x\vert{}A_k) =  \sum_{k=1}^\infty \sum_{x \in \mathbb{R}} xP(A_k) P(X=x\vert{}A_k) = \sum_{k=1}^\infty P(A_k) E(X\vert{}A_k)$.
...
$EX = P(X = 1) E(X\vert{}X=1) + P(X>1) E(X\vert{}X > 1) = p \cdot 1 + (1-p) E(X\vert{}X > 1)$.  

Zauważmy, że $X\vert{}(X > 1)$ ma taki sam rozkład jak $1+X$. Intuicyjnie jest to dość oczywiste, (prosty) formalny dowód dużo ogólniejszego faktu pojawi się na ćwiczeniach.  
...
Aby móc porównywać inwestycje w pierwszym przykładzie i algorytmy w drugim, wprowadzimy miarę tego jak bardzo zmienna losowa odchyla się od swojej wartości średniej. Naturalnym pomysłem byłoby rozważenie wielkości $E\vert{}X-EX\vert{}$. Pomysł ten jest dobry, a tak zdefiniowaną wielkość nazywa się z reguły *średnim odchyleniem $X$*. Posługiwanie się odchyleniem średnim jest jednak z wielu różnych względów dość problematyczne. W dużym uproszczeniu "pojęcie to nie ma dobrych własności", choćby dlatego, że użyta w definicji wartość bezwględna skutecznie utrudnia korzystanie z narzędzi analitycznych takich jak różniczkowanie.
...
To nie wygląda dobrze - sensowna miara średniego odchylenia powinna w takiej sytuacji wzrastać $\vert{}c\vert{}$-krotnie. Rozwiązaniem tego problemu jest odchylenie standardowe, dla którego jak łatwo zauważyć mamy $\sigma(cX) = \vert{}c\vert{}\sigma(X)$.
...
$g_S(t) = E(t^S) = \sum_{k=0}^\infty P(N=k) E(t^S \vert{} N=k) = \sum_{k=0}^\infty P(N=k) E(t^{X_1+\ldots+X_k})$.  
